...
// --- RESUME PROMPT: SAFELY LOADS ON BOOT ---
    ...
    if(savedMatch) {
        showModal("Resume Match?", "An unfinished match was found. Would you like to resume it from where you left off?", function() {
            try {
                ...
                if (state.matchSettings.matchType === 'multiday' && el('breakBtn')) { el('breakBtn').classList.remove('hidden'); }
...
async function authenticateCloudMatch() {
    ...
    // CLEAR OLD DATA WHEN INITIATING A NEW SYNC
    ...
    try {
        ...
        try {
            ...
            battersHtml += battedPlayers.map(item => { 
                let p = item.p, rI = item.i; let isActive = (rI === cur.sIdx || rI === cur.nsIdx); let isStriker = (rI === cur.sIdx); let sr = (p.b && p.b > 0) ? ((p.r / p.b) * 100).toFixed(2) : "0.00"; 
                ...
                let statusInfo = p.out ? `<div style="font-size:0.65rem; color:var(--danger); font-style:italic; margin-top:2px;">${p.dismissalInfo || 'Out'}</div>` : (isActive ? `<div style="font-size:0.65rem; color:var(--success); font-style:italic; margin-top:2px;">Not Out</div>` : '');
                return `<tr style="${rowStyle}"><td style="padding:8px; max-width: 140px;"><div style="font-weight:bold; color:${isActive ? 'white' : 'var(--text-muted)'}; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;" title="${p.name}">${dName}</div>${statusInfo}</td><td style="font-weight:bold; font-size:1.1rem; color:var(--primary); padding:8px;">${p.r || 0}</td><td style="padding:8px;">${p.b || 0}</td><td style="color:var(--b4); padding:8px;">${p.f || 0}</td><td style="color:var(--b6); padding:8px;">${p.s || 0}</td><td style="color:var(--accent); font-weight:bold; padding:8px;">${sr}</td></tr>`; 
            }).join('');
...
function endInnings() { 
    try {
        ...
        // Wait 300ms before forcing the summary UI so memory completely flushes.
        setTimeout(() => { showMatchSummary(); }, 300);
    } catch(e) { 
