...
function openExtra(type) { 
    ...
    let html = `
        <label class="text-primary">Extra Runs:</label>
        <input type="number" id="exR" value="${defVal}" class="modal-input w-100">
    `; 
    if(type === 'NB') { 
...
function generateReportHTML(isExcel) {
    ...
    allInn.forEach(inn => {
        ...
        inn.batters.filter(p => p.hasBatted).forEach(p => { 
            ...
            let status = p.out ? p.dismissalInfo : "Not Out"; 
            let statusColor = p.out ? "#991b1b" : "#065f46"; 
        ...
        inn.bowlers.filter(p => p.o > 0 || p.rc > 0).forEach(p => { 
            ...
            let dName = p.name + (p.desig === 'C' || p.desig === 'C/WK' || p.desig.includes('C') ? ' (C)' : '') + (p.skill && p.skill.includes('WK') ? ' *' : ''); 
            let exStr = `${p.byes||0}b, ${p.legbyes||0}lb`; let noBalls = p.nb || 0; let wides = p.wd || 0; let totalExtras = (p.wd || 0) + (p.nb || 0) + (p.byes || 0) + (p.legbyes || 0); 
            let bBalls = p.o || 0; 
...
function openCardStudio() {
    ...
    let html = `
    <div style="max-height:60vh; overflow-y:auto; padding-right:10px;" class="custom-scroll">
        <button type="button" class="btn-action w-100 mb-15" style="background: linear-gradient(90deg, #3b82f6, #0284c7); padding:15px; font-size:1.1rem; color:white; font-weight:900;" onclick="previewCombinedCard()">📊 GENERATE COMBINED MATCH CARD</button>
        <h3 class="text-accent mt-0" style="border-bottom:1px solid #334155; padding-bottom:5px;">Top Batters</h3>
    ...
    perfs.batters.forEach((b, i) => {
        ...
        html += `
        <button type="button" class="btn-action w-100" style="background:rgba(255,255,255,0.05); text-align:left; padding:12px; border-left:3px solid #38bdf8;" onclick="previewPlayerCard('${pStr}', true, '${cleanName}')">
    ...
    html += `
        ...
        <h3 class="text-accent" style="border-bottom:1px solid #334155; padding-bottom:5px;">Top Bowlers</h3>
    ...
    perfs.bowlers.forEach((b, i) => {
        ...
        let pStr = encodeURIComponent(JSON.stringify(b)).replace(/'/g, "%27");
        html += `
        <button type="button" class="btn-action w-100" style="background:rgba(255,255,255,0.05); text-align:left; padding:12px; border-left:3px solid #ef4444;" onclick="previewPlayerCard('${pStr}', false, '${cleanName}')">
...
function previewPlayerCard(playerStr, isBatter, safeName) {
    ...
    <button type="button" class="btn-action w-100 mt-15" style="background:#10b981; padding:15px; font-size:1.1rem;" onclick="downloadElementAsImage('exportCardElement', 'Sportzstat_Player_${safeName}.png', this)">📸 DOWNLOAD IMAGE</button>
    `;
