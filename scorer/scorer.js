...
function getEffectiveBalls(cur) { let effective = 0; let prevActual = 0; for (let i = 0; i < cur.overHistory.length; i++) { let actualBallsInOver = cur.overHistory[i].totalBallsAtEnd - prevActual; if (cur.overHistory[i].isPartialTerminal) { effective += actualBallsInOver; } else { effective += 6; } prevActual = cur.overHistory[i].totalBallsAtEnd; } let currentOverBalls = Math.max(0, cur.balls - prevActual); effective += currentOverBalls; return effective; }
function getTimeDropdownsHtml(prefix, allowNA = false) { let d = new Date(); let curH = d.getHours(); let curM = d.getMinutes(); let ampm = curH >= 12 ? 'PM' : 'AM'; curH = curH % 12; if(curH === 0) curH = 12; let h = allowNA ? '<option value="">--</option>' : ''; let m = allowNA ? '<option value="">--</option>' : ''; for(let i=1; i<=12; i++) { let val = i.toString().padStart(2,'0'); let sel = (!allowNA && i === curH) ? 'selected' : ''; h += `<option value="${val}" ${sel}>${val}</option>`; } for(let i=0; i<60; i++) { let val = i.toString().padStart(2,'0'); let sel = (!allowNA && i === curM) ? 'selected' : ''; m += `<option value="${val}" ${sel}>${val}</option>`; } let amSel = (!allowNA && ampm === 'AM') ? 'selected' : ''; let pmSel = (!allowNA && ampm === 'PM') ? 'selected' : ''; return `<div class="flex-row gap-5"><select id="${prefix}Hr" class="modal-input" style="width:33%; margin-bottom:0;">${h}</select><select id="${prefix}Min" class="modal-input" style="width:33%; margin-bottom:0;">${m}</select><select id="${prefix}AmPm" class="modal-input" style="width:33%; margin-bottom:0;">${allowNA ? '<option value="">--</option>' : ''}<option value="AM" ${amSel}>AM</option><option value="PM" ${pmSel}>PM</option></select></div>`; }
...
async function triggerCloudSync() {
    ...
    try { const { error } = await supabaseClient.from('live_matches').upsert({ match_id: state.matchId, live_data: lightWeightLiveData }, { onConflict: 'match_id' }); if (error) console.error("Cloud sync error: ", error); } catch(e) {}
}
...
async function logCareerStats() { if (!supabaseClient || !state.matchId) return; let payloadStr = JSON.stringify(state.inningsSummaries); try { await supabaseClient.from('completed_matches').upsert({ match_id: state.matchId, final_data: payloadStr }, { onConflict: 'match_id' }); } catch (e) {} }
...
function processExtraSubmit() {
    ...
    if(typ === 'W') { let t = ex + 1; cur.runs += t; cur.runsInThisOver += t; cur.currPartnership.runs += t; b.rc += t; b.wd += t; cur.extras.w += t; lbl = t + 'wd'; runsExt = t; exLabel = 'Wide'; } 
        ...
        let nt = el('nbT').value; cur.runs += 1; cur.extras.nb += 1; b.rc += 1; b.nb += 1; cur.runsInThisOver += 1; cur.currPartnership.runs += (1 + ex); runsExt = 1; exLabel = 'No-Ball';
...
function processWicketSubmit() {
    ...
    if (wType === 'RetiredHurt' || wType === 'RetiredOut') { 
        ...
        cur.currentOverLog.push({label: 'Ret', type: 'wicket'}); logBallEvent(s, b, 0, 0, 'None', true, wType, oB);
    } else {
        cur.wkts++; oB.out = true; 
        let runsScored = 0, runType = 'bat'; let runsExt = 0; let exLabel = 'None';
        
        if (['RunOut', 'ObstructingField'].includes(wType)) { 
            let runsInput = el('wRuns') ? el('wRuns').value : "0"; runsScored = parseInt(runsInput, 10); if (isNaN(runsScored)) runsScored = 0; 
...
function updateUI() {
    ...
    } else { el('activeBowlerNameRight').innerText = "Select..."; el('activeBowlerNameRight').title = ""; el('activeBowlerProgress').innerHTML = ""; el('miniLiveBowler').innerHTML = `<div style="color: var(--text-muted); font-style:italic;">Select Bowler...</div>`; }
...
function calculateResultText() { 
    ...
    if (t1C + t2C < 3) return "";
    if (lInn.wkts >= 10 || state.inningsNum > state.matchSettings.maxInnings) { if (s[batL] > s[bowlL]) return `${batL} won by ${s[batL] - s[bowlL]} runs`; if (s[bowlL] > s[batL] && t1C+t2C===4) return `${bowlL} won by ${10 - lInn.wkts} wickets`; } return ""; 
}
...
function getTopPerformers() {
    let batters = [], bowlers = [], allInn = [...state.inningsSummaries]; 
    ...
    allInn.forEach(inn => { inn.batters.forEach(b => { if(b.hasBatted && b.r >= 10) batters.push({...b, team: inn.batTeam}); }); inn.bowlers.forEach(b => { if(b.w >= 1 || b.m >= 1) bowlers.push({...b, team: inn.bowlTeam}); }); });
...
function openCardStudio() {
    ...
    perfs.bowlers.forEach((b, i) => { let pStr = encodeURIComponent(JSON.stringify(b)).replace(/'/g, "%27"); html += `<button type="button" class="btn-action w-100" style="background:rgba(255,255,255,0.05); text-align:left; padding:12px; border-left:3px solid #ef4444;" onclick="previewPlayerCard('${pStr}', false)"><b>${b.name}</b> (${b.team}) - <span class="text-danger">${b.w} Wkts</span> for ${b.rc} runs</button>`; });
    html += `</div></div>`; showModal("🌟 PLAYER CARD STUDIO", html, () => {}, true, "400px"); el('modalConfirmBtn').style.display = 'none'; el('modalCancelBtn').style.display = ''; el('modalCancelBtn').innerText = "🔙 Back to Summary"; el('modalCancelBtn').onclick = () => { showMatchSummary(); };
}

function previewCombinedCard() {
    let perfs = getTopPerformers(); let tournName = el('setupTournament').value || "OFFICIAL MATCH CARD";
...
function previewPlayerCard(playerStr, isBatter) {
    let p = JSON.parse(decodeURIComponent(playerStr)); let stat1Val, stat1Lab, stat2Val, stat2Lab, stat3Val, stat3Lab;
    if (isBatter) { let sr = p.b > 0 ? ((p.r/p.b)*100).toFixed(2) : "0.00"; stat1Val = p.r + (p.out ? "" : "*"); stat1Lab = "RUNS"; stat2Val = p.b; stat2Lab = "BALLS"; stat3Val = sr; stat3Lab = "STRIKE RATE"; } else { let econ = p.o > 0 ? ((p.rc/p.o)*6).toFixed(2) : "0.00"; stat1Val = p.w; stat1Lab = "WICKETS"; stat2Val = formatOver(p.o); stat2Lab = "OVERS"; stat3Val = econ; stat3Lab = "ECONOMY"; }
    ...
    <div style="width: 302px; height: 378px; position: relative; margin: 0 auto; overflow: hidden; border-radius: 10px; box-shadow: 0 4px 15px rgba(0,0,0,0.8);">
        <div id="exportCardElement" style="width: 1080px; height: 1350px; position: absolute; top: 0; left: 0; transform-origin: top left; transform: scale(0.2796); background-color: #111; font-family: 'Helvetica Neue', Arial, sans-serif; box-sizing: border-box; display: flex; flex-direction: column; padding: 40px; overflow: hidden;">
            <div style="position: absolute; top: -5%; left: -5%; width: 110%; height: 110%; background-image: url('https://images.unsplash.com/photo-1540747913346-19e32dc3e97e?auto=format&fit=crop&w=1080&q=80'); background-size: cover; background-position: center; filter: blur(12px); z-index: 0;"></div><div style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: rgba(10, 15, 30, 0.75); z-index: 1;"></div>
            <div style="position:relative; z-index:2; display:flex; flex-direction:column; height:100%;">
                <div style="display:flex; justify-content:space-between; align-items:flex-start; width: 100%; margin-bottom: 20px;"><div style="font-weight: 900; font-size: 2rem; color: #ffffff; text-transform: uppercase; letter-spacing: 2px; text-shadow: 2px 2px 4px rgba(0,0,0,0.8);">PLAYER CONTRIBUTION</div><div style="font-weight: 900; font-size: 2.5rem; color: #ffffff; text-transform: uppercase; text-shadow: 2px 2px 4px rgba(0,0,0,0.8);">${p.team}</div></div>
                <div style="display: flex; width: 100%; height: 100%; gap: 40px;">
                    ...
                    <div style="width: 45%; display: flex; flex-direction: column; justify-content: center; align-items: center; gap: 70px; padding-bottom: 80px;">
                        <div style="display: flex; flex-direction: column; align-items: center;"><div style="font-family: 'Impact', 'Arial Black', sans-serif; font-size: 12rem; color: #ffffff; line-height: 1; transform: scaleY(1.2); letter-spacing: -2px; text-shadow: 5px 5px 15px rgba(0,0,0,0.8);">${stat1Val}</div><div style="color: #FFCC00; font-size: 2.2rem; font-weight: 900; text-transform: uppercase; margin-top: 25px; letter-spacing: 4px; text-shadow: 2px 2px 5px rgba(0,0,0,0.9);">${stat1Lab}</div></div>
                        <div style="display: flex; flex-direction: column; align-items: center;"><div style="font-family: 'Impact', 'Arial Black', sans-serif; font-size: 12rem; color: #ffffff; line-height: 1; transform: scaleY(1.2); letter-spacing: -2px; text-shadow: 5px 5px 15px rgba(0,0,0,0.8);">${stat2Val}</div><div style="color: #FFCC00; font-size: 2.2rem; font-weight: 900; text-transform: uppercase; margin-top: 25px; letter-spacing: 4px; text-shadow: 2px 2px 5px rgba(0,0,0,0.9);">${stat2Lab}</div></div>
                        <div style="display: flex; flex-direction: column; align-items: center;"><div style="font-family: 'Impact', 'Arial Black', sans-serif; font-size: 11rem; color: #ffffff; line-height: 1; transform: scaleY(1.2); letter-spacing: -2px; text-shadow: 5px 5px 15px rgba(0,0,0,0.8);">${stat3Val}</div><div style="color: #FFCC00; font-size: 2.2rem; font-weight: 900; text-transform: uppercase; margin-top: 25px; letter-spacing: 4px; text-shadow: 2px 2px 5px rgba(0,0,0,0.9);">${stat3Lab}</div></div>
                    </div>
                </div>
...
function downloadElementAsImage(elementId, fileName, btn) {
    ...
    setTimeout(() => { html2canvas(clone, { scale: 1, backgroundColor: "#111", useCORS: true, allowTaint: true }).then(canvas => { let link = document.createElement('a'); link.download = fileName; link.href = canvas.toDataURL("image/png"); link.click(); document.body.removeChild(clone); document.body.removeChild(overlay); btn.innerText = origText; btn.disabled = false; }).catch(err => { console.error(err); alert("Error generating image."); document.body.removeChild(clone); document.body.removeChild(overlay); btn.innerText = origText; btn.disabled = false; }); }, 150);
}
...
function showMatchSummary() {
    ...
    if (state.inningsSummaries.length === 0 && state.current.balls === 0) { html += `<p class="text-center text-muted">No data available yet.</p>`; } else {
    ...
    html += `</div><div style="border-top:1px solid var(--border); padding-top:10px;"><label class="text-accent">Official Match Result / Status</label><input type="text" id="finalMatchResult" class="modal-input w-100" value="${state.matchResult || autoRes}" placeholder="e.g., Match Awarded, Follow-on, etc."><label class="text-accent mt-5">Allowances for Inning (Mins)</label><input type="number" id="inningAllowancesInput" class="modal-input w-100" placeholder="e.g. 15" value="${currentAllowances}"><div class="flex-row gap-10 mt-10 mb-10"><button type="button" onclick="downloadSummaryExcel(); enableSummaryConfirm();" class="btn-action w-100" style="background:#0284c7; padding:12px; font-size:1rem; box-shadow: 0 4px 10px rgba(0,0,0,0.5);">📥 EXCEL</button><button type="button" onclick="downloadSummaryPDF(); enableSummaryConfirm();" class="btn-action w-100" style="background:#be123c; padding:12px; font-size:1rem; box-shadow: 0 4px 10px rgba(0,0,0,0.5);">🖨️ PDF / PRINT</button></div><button type="button" onclick="openCardStudio()" class="btn-action w-100 mb-10" style="background: linear-gradient(90deg, #f59e0b, #d97706); padding:15px; font-size:1.1rem; color:black; font-weight:900; box-shadow: 0 4px 10px rgba(0,0,0,0.5);">🌟 OPEN PLAYER CARD STUDIO</button></div>`;
             
    showModal(isGameOver ? "🏁 MATCH COMPLETE" : (isTransition ? `🛑 END OF INNINGS ${state.inningsNum}` : "📋 DETAILED MATCH SCORECARD"), html, () => { state.matchResult = el('finalMatchResult') ? el('finalMatchResult').value : autoRes; if(el('inningAllowancesInput')) { let val = parseInt(el('inningAllowancesInput').value) || 0; state.current.allowances = val; if (state.inningsSummaries.length > 0 && state.inningsNum === state.inningsSummaries.length) { state.inningsSummaries[state.inningsSummaries.length - 1].allowances = val; } } closeModal(); setTimeout(() => { if (isGameOver) { logCareerStats(); setTimeout(() => { resetMatch(); }, 1000); } else if (isTransition) { openTransitionManager(); } }, 300); }, hideCancel, "700px", confirmBtnText, requiresDownload); el('modalCancelBtn').innerText = "🔙 Go Back & Edit";
}

function endInnings() { 
    try { saveState(); if (state.current.bIdx !== null) finalizeOver(true); state.current.inningsEndTime = new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}); let fF = JSON.parse(JSON.stringify(state.current.fow || [])); if(state.current.sIdx !== null && state.current.nsIdx !== null && state.current.wkts < 10) { let sBName = getBatTeam().players[state.current.sIdx] ? getBatTeam().players[state.current.sIdx].name : "Unknown"; let nsBName = getBatTeam().players[state.current.nsIdx] ? getBatTeam().players[state.current.nsIdx].name : "Unknown"; let pRuns = state.current.currPartnership ? state.current.currPartnership.runs : 0; let pBalls = state.current.currPartnership ? state.current.currPartnership.balls : 0; fF.push({ wktNum: "Unbroken", runs: state.current.runs, overs: formatOver(state.current.balls), outBatter: "-", partner: sBName + " & " + nsBName, pRuns: pRuns, pBalls: pBalls }); } state.inningsSummaries.push({ innNum: state.inningsNum, batTeam: getBatTeam().name, bowlTeam: getBowlTeam().name, runs: state.current.runs, wkts: state.current.wkts, overs: formatOver(state.current.balls), penalties: state.current.penalties || 0, overHistory: JSON.parse(JSON.stringify(state.current.overHistory || [])), batters: JSON.parse(JSON.stringify(getBatTeam().players || [])), bowlers: JSON.parse(JSON.stringify(getBowlTeam().players || [])), fow: fF, extras: JSON.parse(JSON.stringify(state.current.extras)), startTime: state.current.inningsStartTime || "-", endTime: state.current.inningsEndTime, allowances: state.current.allowances || 0 }); showMatchSummary(); } catch(e) { console.error("End Innings Error:", e); alert("Error saving Innings Summary. Check console."); }
}
...
function executeTransition(nBat, nBowl, action) { 
    ...
    ['A', 'B'].forEach(t => state.teams[t].players.forEach(p => { p.r = pI seem to be encountering an error. Can I try something else for you?
