...
function confirmStartMatch() {
    let strikerEl = document.getElementById('gwStriker');
    let nonStrikerEl = document.getElementById('gwNonStriker');
    let bowlerEl = document.getElementById('gwBowler');

    if(!strikerEl || !nonStrikerEl || !bowlerEl) {
        alert("CRITICAL ERROR: HTML and JavaScript are out of sync. Please replace your live.html code.");
        return;
    }

    let sUUID = strikerEl.value;
    let nsUUID = nonStrikerEl.value;
    let bUUID = bowlerEl.value;
...
function checkAutoOverPrompt() {
    ...
    if (currentOverBalls === 6) {
        let strikerOut = cur.sIdx !== null && getBatTeam().players[cur.sIdx] && getBatTeam().players[cur.sIdx].out;
        let nonStrikerOut = cur.nsIdx !== null && getBatTeam().players[cur.nsIdx] && getBatTeam().players[cur.nsIdx].out;
...
function generateManualGrids() {
    ...
    function buildGridRows(teamName) { let rows = `<tr><th>Player Name</th><th>Reg No</th><th title="Not Out">NO*</th><th title="Runs">R</th><th title="Balls">B</th><th>4s</th><th>6s</th><th title="Overs (e.g. 2.3)">Ov</th><th title="Maidens">M</th><th title="Runs Conceded">R(Bwl)</th><th title="Wickets">W</th><th title="Wides Bowled">Wd</th><th title="No Balls Bowled">NB</th><th title="Catches Taken">Ct</th><th title="Stumpings Made">St</th><th title="Run Outs Effected">RO</th></tr>`; for(let i=0; i<15; i++) { let pfx = (teamName===tA ? "mea" : "meb") + "-" + i; rows += `<tr><td><input type="text" id="${pfx}-name" placeholder="Player ${i+1}" style="width:120px;"></td><td><input type="text" id="${pfx}-reg" placeholder="Reg No" style="width:60px;"></td><td style="text-align:center;"><input type="checkbox" id="${pfx}-no" style="width:16px;height:16px; cursor:pointer;" title="Check if Not Out"></td><td><input type="number" id="${pfx}-r" class="m-input-num" min="0"></td><td><input type="number" id="${pfx}-b" class="m-input-num" min="0"></td><td><input type="number" id="${pfx}-4s" class="m-input-num" min="0"></td><td><input type="number" id="${pfx}-6s" class="m-input-num" min="0"></td><td><input type="number" id="${pfx}-o" class="m-input-num" min="0" step="0.1" onblur="fixOversInput(this)"></td><td><input type="number" id="${pfx}-m" class="m-input-num" min="0"></td><td><input type="number" id="${pfx}-br" class="m-input-num" min="0"></td><td><input type="number" id="${pfx}-w" class="m-input-num" min="0"></td><td><input type="number" id="${pfx}-wd" class="m-input-num" min="0"></td><td><input type="number" id="${pfx}-nb" class="m-input-num" min="0"></td><td><input type="number" id="${pfx}-ct" class="m-input-num" min="0"></td><td><input type="number" id="${pfx}-st" class="m-input-num" min="0"></td><td><input type="number" id="${pfx}-ro" class="m-input-num" min="0"></td></tr>`; } return rows; }
...
function openRemarkModal() { 
    ...
    let fOpts = `<option value="">-- No Fielder / N/A --</option>`; getBowlTeam().players.forEach(p => { if(p.name && p.name !== "Empty Slot") fOpts += `<option value="${p.name}">${p.name}</option>`; }); 
...
function markOpenerTimes() { 
    if (state.current.sIdx !== null && state.current.nsIdx !== null) { 
        ...
        if (getBatTeam().players[state.current.sIdx] && !getBatTeam().players[state.current.sIdx].inTime) getBatTeam().players[state.current.sIdx].inTime = now; 
        if (getBatTeam().players[state.current.nsIdx] && !getBatTeam().players[state.current.nsIdx].inTime) getBatTeam().players[state.current.nsIdx].inTime = now; 
    } 
}
...
function processWicketSubmit() {
    ...
    if (wType === 'RetiredHurt' || wType === 'RetiredOut') { 
        ...
        if (wType === 'Stumped') { let wk = getBowlTeam().players.find(p => p.skill && String(p.skill).includes('WK') && p.isPlayingXI); if (wk) { wk.stumpings++; wFldr = wk.name; } else { wFldr = "WK"; } } else if (wFldr) { let fObj = getBowlTeam().players.find(p => p.name === wFldr); if (fObj) { if (wType === 'Caught') fObj.catches++; else if (wType === 'RunOut') fObj.runouts++; } }
...
function openChangeWK() { 
    let o = getBowlTeam().players.map((p, i) => (p.isPlayingXI && p.name !== "Empty Slot") ? `<option value="${i}" ${p.skill && String(p.skill).includes('WK') ? 'selected' : ''}>${p.name}</option>` : '').join(''); 
...
function updateUI() {
    try {
        let cur = state.current, bT = getBatTeam() ? getBatTeam().players : [];
