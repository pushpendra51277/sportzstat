...
async function authenticateCloudMatch() {
    ...
    // CLEAR OLD MATCHES WHEN SYNCING
...
async function fetchCloudRosters(teamKey, teamName) {
    ...
    if (error || !data || data.length === 0) {
        listEl.innerHTML = `<div style="color: #ef4444; font-size: 0.85rem; font-style: italic;">No official roster found in cloud.</div>`;
        return;
    }
...
function refreshOpeningDropdowns() {
    ...
    if (batChecked.length > 0) {
        if(sEl) sEl.innerHTML = batChecked.map((p, idx) => `<option value="${p.id}" ${p.id === curS \vert{}\vert{} (!curS && idx===0)?'selected':''}>${p.name}</option>`).join('');
        if(nsEl) nsEl.innerHTML = batChecked.map((p, idx) => `<option value="${p.id}" ${p.id === curNS \vert{}\vert{} (!curNS && idx===1)?'selected':''}>${p.name}</option>`).join('');
    } else {
    ...
    if (bowlChecked.length > 0) {
        if(bEl) bEl.innerHTML = bowlChecked.map((p, idx) => `<option value="${p.id}" ${p.id === curB \vert{}\vert{} (!curB && idx===0)?'selected':''}>${p.name}</option>`).join('');
    } else {
...
// Aliases for mismatched HTML triggers
...
function closeModal() { 
    el('dynamicModal').classList.add('hidden'); el('dynamicModal').style.display = 'none';
    let scoringBox = el('scoringEventsBox'); if (scoringBox) { scoringBox.style.pointerEvents = 'auto'; scoringBox.style.opacity = '1'; }
}
...
function calculateResultText() { 
    let allInn = [...state.inningsSummaries]; 
    if (state.current && (state.current.balls > 0 || state.current.runs > 0) && state.inningsNum > state.inningsSummaries.length) { allInn.push({ innNum: state.inningsNum, batTeam: getBatTeam().name, bowlTeam: getBowlTeam().name, runs: state.current.runs, wkts: state.current.wkts }); }
        ...
        if (state.inningsSummaries.length === 0 && (!state.current || state.current.balls === 0)) {
            ...
            displayInnings.reverse().forEach(inn => {
                ...
                if(inn.batters) {
                    inn.batters.forEach(b => { 
                        if(b && b.hasBatted && b.name !== "Empty Slot") { 
                            ...
                            let nameStr = `<div style="max-width: 130px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;" title="${b.name \vert{}\vert{} ''}"><b style="font-size:0.85rem; color:white;">${b.name || 'Unknown'}</b>` + (b.desig==='C' || b.desig==='C/WK' ? ' <span style="color:var(--accent);">(C)</span>' : '') + (b.skill && String(b.skill).includes('WK') ? ' 🧤' : '') + `</div>`; 
                            html += `<tr><td>${nameStr}${isOut}</td><td style="font-weight:bold; font-size:1rem; color:white;">${b.r || 0}</td><td style="color:white;">${b.b \vert{}\vert{} 0}</td><td style="color:white;">${b.f || 0}</td><td style="color:white;">${b.s \vert{}\vert{} 0}</td><td style="color:var(--accent);">${sr}</td></tr>`; 
                        } 
                    });
                }
                
                let ex = inn.extras || {w:0, nb:0, b:0, lb:0}; let extrasTotal = (ex.w||0) + (ex.nb||0) + (ex.b||0) + (ex.lb||0); let pen = inn.penalties || 0;
                html += `<tr style="background:rgba(255,255,255,0.05); font-weight:bold;"><td style="color:var(--accent); text-transform:uppercase;">Extras</td><td colspan="5" style="text-align:right; color:white;">${extrasTotal} <span style="font-weight:normal; font-size:0.7rem; color:white;">(W:${ex.w\vert{}\vert{}0}, NB:${ex.nb||0}, B:${ex.b\vert{}\vert{}0}, LB:${ex.lb||0})</span></td></tr>`;
                ...
                if(inn.bowlers) {
                    inn.bowlers.forEach(b => { 
                        if(b && (b.o > 0 || b.rc > 0) && b.name !== "Empty Slot") { 
                            ...
                            let nameStr = `<div style="max-width: 100px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;" title="${b.name \vert{}\vert{} ''}"><b style="color:white;">${b.name || 'Unknown'}</b></div>`; 
                            let exStr = `${b.byes\vert{}\vert{}0}b,${b.legbyes||0}lb`; 
                            let nb = b.nb || 0; let wd = b.wd || 0; 
                            html += `<tr><td>${nameStr}</td><td style="color:white;">${formatOver(b.o\vert{}\vert{}0)}</td><td style="color:white;">${b.m||0}</td><td style="color:white;">${totalRuns}</td><td style="font-weight:bold; font-size:1rem; color:var(--danger);">${b.w||0}</td><td style="color:var(--accent);">${econ}</td><td style="font-size:0.75rem; color:white;">${exStr}</td><td style="color:white;">${nb}</td><td style="color:white;">${wd}</td></tr>`; 
                        } 
                    });
                }
                ...
                if(inn.fow && inn.fow.length > 0) { 
                    let fowStr = inn.fow.map(f => `<b style="color:white;">${f.runs \vert{}\vert{} 0}/${f.wktNum==='Unbroken'?'*':f.wktNum}</b> <span style="font-size:0.65rem; color:white;">(${f.outBatter \vert{}\vert{} ''},${f.overs || '0.0'} ov)</span>`).join(', '); 
...
function openChangeWK() { let o = getBowlTeam().players.map((p, i) => (p.isPlayingXI && p.name !== "Empty Slot") ? `<option value="${i}" ${p.skill && String(p.skill).includes('WK') ? 'selected' : ''}>${p.name}</option>` : '').join(''); showModal("🧤 Change WK", `<select id="nWK" class="modal-input w-100"><option value="-1">-- No WK Selected --</option>${o}</select>`, () => { getBowlTeam().players.forEach(p => { if(p.skill && String(p.skill).includes('/ WK')) { p.skill = p.skill.replace(' / WK', ''); } }); let idx = parseInt(el('nWK').value); if(idx >= 0) { if(!getBowlTeam().players[idx].skill.includes('WK')) { getBowlTeam().players[idx].skill += ' / WK'; } } syncMetadataToHistory(false); closeModal(); updateUI(); }); }
...
function openSelector(typ, title) { let cur = state.current, lst = typ === 'bowler' ? getBowlTeam().players : getBatTeam().players; let opt = lst.map((p, i) => { if (!p.isPlayingXI || p.name === "Empty Slot") return ''; let s = typ === 'bowler' ? (!cur.lastOverBowlers.has(i) && i !== cur.bIdx && p.quotaOvers < state.matchSettings.bowlerQuota) : ((!p.hasBatted || p.out === 'retiredHurt') && i !== cur.sIdx && i !== cur.nsIdx); return s ? `<option value="${i}">${p.name}</option>` : ''; }).join(''); showModal(title, `<select id="gSel" class="modal-input w-100">${opt}</select>`, () => { saveState(); let val = parseInt(el('gSel').value); if(typ === 'bowler') { cur.bIdx = val; cur.bowlersInCurrentOver.add(val); } else { cur[typ === 'striker' ? 'sIdx' : 'nsIdx'] = val; let nB = getBatTeam().players[val]; if (nB.out === 'retiredHurt') { nB.out = false; nB.dismissalInfo = ""; } else { nB.inTime = new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}); } nB.hasBatted = true; } closeModal(); updateUI(); if (typ !== 'bowler' && cur.bIdx === null && cur.wkts < 10) { setTimeout(() => openSelector('bowler', "Select Next Bowler"), 50); } else if (typ !== 'bowler') { checkAutoOverPrompt(); } }, false, "360px", "Confirm"); }
...
function resetMatch() { state.matchBreaks = []; state.inningsNum = 1; state.inningsSummaries = []; state.matchResult = ""; stateHistory = []; remarkLog = []; state.matchId = ""; state.current = { runs:0, wkts:0, balls:0, sIdx:null, nsIdx:null, bIdx:null, isFreeHit: false, penalties: 0, lastOverBowlers: new Set(), extras: {w:0, nb:0, b:0, lb:0}, recentBalls: [], currentOverLog: [], runsInThisOver: 0, bowlersInCurrentOver: new Set(), overHistory: [], currPartnership: { runs: 0, balls: 0 }, fow: [], activeBreak: null, activeBreakStartTime: null, activeBreakInsp: null, pendingBreakMins: 0, inningsStartTime: null, inningsEndTime: null, allowances: 0 }; el('inningsHistoryText').innerHTML = ''; el('scoringView').classList.add('hidden'); el('setupView').classList.remove('hidden'); localStorage.removeItem('cricStat_activeMatch'); if (window.matchChart) window.matchChart.destroy(); el('setupMatchId').value = ''; if (el('setupDate').value) { generateMatchId(); } }
...
function showMatchGraphs() { showModal("📊 MATCH GRAPHS", `<div class="flex-row gap-10" style="justify-content:center"><button type="button" onclick="renderChart('worm')" class="btn-I'm having a hard time fulfilling your request. Can I help you with something else instead?
