...
function startInnings() {
    ...
    if (activeMatch && activeMatch.full_state) {
        ...
        if (format.includes('Multi-Day') || format === 'Test') {
            state.matchSettings.matchType = 'multiday'; state.matchSettings.maxOvers = 999; state.matchSettings.bowlerQuota = 999; state.matchSettings.maxInnings = 4; el('breakBtn').classList.remove('hidden');
...
function saveState() { 
    try { stateHistory.push(JSON.stringify(state, (key, value) => value instanceof Set ? [...value] : value)); if (stateHistory.length > 100) stateHistory.shift(); localStorage.setItem('cricStat_stateHistory', JSON.stringify(stateHistory)); } 
    catch(e) { console.warn("State save failed"); } 
}

function undoLastAction() { 
    if (stateHistory.length > 0) { 
        let prevState = JSON.parse(stateHistory.pop()); prevState.current.lastOverBowlers = new Set(prevState.current.lastOverBowlers); prevState.current.bowlersInCurrentOver = new Set(prevState.current.bowlersInCurrentOver); state = prevState; 
        localStorage.setItem('cricStat_stateHistory', JSON.stringify(stateHistory)); updateUI(); 
...
function openExtra(type) { 
    ...
    if(type === 'NB') { html += `<select id="nbT" class="modal-input w-100 mt-5"><option value="bat">Off the Bat</option><option value="bye">Byes</option><option value="legbye">Leg Byes</option></select>`; }
...
function processExtraSubmit() {
    ...
    if(typ === 'W') { 
        let t = ex + 1; cur.runs += t; cur.runsInThisOver += t; cur.currPartnership.runs += t; b.rc += t; b.wd += t; cur.extras.w += t; lbl = t + 'wd'; runsExt = t; exLabel = 'Wide';
    } else if (typ === 'NB') { 
...
function openRetire() { 
    if(state.current.bIdx === null) return openSelector('bowler', "Select Bowler"); 
    let html = `<label class="text-primary">Batter Out</label><select id="wWho" class="modal-input w-100"><option value="striker">Striker</option><option value="nonstriker">Non-Striker</option></select><label class="text-primary mt-5">Dismissal Type</label><select id="wType" class="modal-input w-100"><option value="RetiredHurt">Retired - Not Out</option><option value="RetiredOut">Retired - Out</option></select>`;
...
function processWicketSubmit() {
    ...
    if (wType === 'RetiredHurt' || wType === 'RetiredOut') { 
        ...
        if (extraType === 'wide') dT += ' (wd)'; else if (extraType === 'noball') dT += ' (nb)'; oB.dismissalInfo = dT; logBallEvent(s, b, runsBat, runsExt, exLabel, true, wType, oB); 
    }
...
function openPenalty() { let html = `<select id="penT" class="modal-input w-100"><option value="bat">Batting Team</option><option value="bowl">Bowling Team</option></select><input type="number" id="penR" class="modal-input w-100" placeholder="Runs" value="5">`; showModal("Award Penalty Runs", html, () => { saveState(); let t = el('penT').value, r = parseInt(el('penR').value) || 0; if (r === 0) { closeModal(); return; } if (t === 'bat') { state.current.runs += r; state.current.penalties += r; state.current.currentOverLog.push({label: `+${r}P`, type: 'extra'}); if (checkTargetReached()) { finalizeOver(true); closeModal(); setTimeout(endInnings, 100); return; } } else { let p = state.inningsSummaries.find(i => i.batTeam === getBowlTeam().name); if (p) { p.runs += r; p.penalties = (p.penalties || 0) + r; alert("Penalty added to completed innings!"); } else { state.teams[state.bowlingKey].pendingPenalties += r; alert("Penalty will be added to next innings!"); } } closeModal(); updateUI(); }); }
...
function openPlayerSwap() { let html = `<select id="swT" class="modal-input w-100" onchange="updSwap()"><option value="A">${state.teams.A.name}</option><option value="B">${state.teams.B.name}</option></select><label>OUT:</label><select id="swO" class="modal-input w-100"></select><label>IN:</label><select id="swI" class="modal-input w-100"></select>`; showModal("🔄 Sub Swap", html, () => { let k = el('swT').value, o = parseInt(el('swO').value), i = parseInt(el('swI').value), t = state.teams[k].players; if ((k === state.battingKey && (state.current.sIdx === o || state.current.nsIdx === o)) || (k === state.bowlingKey && state.current.bIdx === o)) { alert("Cannot swap active player!"); return; } saveState(); t[o].isPlayingXI = false; if (!t[o].dismissalInfo && t[o].hasBatted) t[o].dismissalInfo = "Replaced (Injury)"; t[i].isPlayingXI = true; closeModal(); updateUI(); }); setTimeout(updSwap, 10); }
...
function updateUI() {
    ...
    el('bowlStatsBody').innerHTML = bwT.filter(p => p.o > 0 || p.rc > 0).map(p => { 
        ...
        let exStr = `${p.byes\vert{}\vert{}0}b,${p.legbyes||0}lb`; let noBalls = p.nb || 0; let wides = p.wd || 0;
...
function generateReportHTML(isExcel) {
    let css = `
        @media print { @page { size: A4 landscape; margin: 0.5in; } body { -webkit-print-color-adjust: exact; print-color-adjust: exact; } } 
        body { font-family: 'Segoe UI', Calibri, Arial, sans-serif; background: #ffffff; padding: 20px; } 
        table { border-collapse: collapse; width: 100%; font-size: 10pt; table-layout: auto; margin-bottom: 20px; page-break-inside: avoid; } 
        tr { page-break-inside: avoid; page-break-after: auto; } 
        th, td { border: 1px solid #d1d5db; padding: 6px; text-align: center; vertical-align: middle; color: #334155; } 
        .main-header { background: #0f172a; color: #ffffff; font-size: 14pt; font-weight: bold; text-transform: uppercase; padding: 10px; } 
        .sub-header { background: #f8fafc; color: #334155; font-size: 10pt; font-weight: bold; text-align: left; padding: 8px; } 
        .inn-title { background: #1e293b; color: #fbbf24; font-size: 12pt; font-weight: bold; text-align: left; padding: 8px; } 
        .bat-th, .bwl-th { background: #f1f5f9; color: #334155; font-weight: bold; } 
        .text-left { text-align: left; padding-left: 10px; } 
        .text-right { text-align: right; padding-right: 10px; } 
        .bold { font-weight: bold; } 
        .extra-row { background: #f1f5f9; font-weight: bold; color: #334155; border-top: 2px solid #94a3b8; } 
        .fow-row { background: #fafafa; font-size: 9pt; color: #475569; text-align: left; padding: 10px; }
    `;
    let html = ``;
    ...
    if ((state.current.balls > 0 || state.current.runs > 0) && state.inningsNum > state.inningsSummaries.length) { 
        ...
        allInn.push({ innNum: state.inningsNum, batTeam: getBatTeam().name, bowlTeam: getBowlTeam().name, runs: state.current.runs, wkts: state.current.wkts, overs: formatOver(state.current.balls), penalties: state.current.penalties || 0, batters: getBatTeam().players, bowlers: getBowlTeam().players, fow: fF, extras: JSON.parse(JSON.stringify(state.current.extras)), isOngoing: true, allowances: state.current.allowances || 0 }); 
    }
    ...
    allInn.forEach(inn => {
        ...
        inn.batters.filter(p => p.hasBatted).forEach(p => { 
            let sr = p.b > 0 ? ((p.r / p.b) * 100).toFixed(2) : "0.00"; 
            let dName = p.name + (p.desig === 'C' || p.desig === 'C/WK' || p.desig.includes('C') ? ' (C)' : '') + (p.skill && p.skill.includes('WK') ? ' *' : ''); 
            let status = p.out ? p.dismissalInfo : "Not Out"; let statusColor = p.out ? "#991b1b" : "#065f46"; 
            html += `<tr><td colspan="2" class="text-left bold">${dName}</td><td colspan="2" style="color:${statusColor}; font-size:9pt;">${status}</td><td class="bold">${p.r}</td><td>${p.b}</td><td>${p.f}</td><td>${p.s}</td><td colspan="3">${sr}</td></tr>`; 
        });
        ...
        inn.bowlers.filter(p => p.o > 0 || p.rc > 0).forEach(p => { 
            ...
            let dName = p.name + (p.desig === 'C' || p.desig === 'C/WK' || p.desig.includes('C') ? ' (C)' : '') + (p.skill && p.skill.includes('WK') ? ' *' : ''); 
            let exStr = `${p.byes||0}b, ${p.legbyes||0}lb`; let noBalls = p.nb || 0; let wides = p.wd || 0; let totalExtras = (p.wd || 0) + (p.nb || 0) + (p.byes || 0) + (p.legbyes || 0); let bBalls = p.o || 0; 
...
function openCardStudio() {
    ...
    perfs.batters.forEach((b, i) => {
        let cleanName = b.name.replace(/[^a-zA-Z0-9]/g, '_'); let pStr = encodeURIComponent(JSON.stringify(b)).replace(/'/g, "%27");
        html += `<button type="button" class="btn-action w-100" style="background:rgba(255,255,255,0.05); text-align:left; padding:12px; border-left:3px solid #38bdf8;" onclick="previewPlayerCard('${pStr}', true, '${cleanName}')"><b>${b.name}</b> (${b.team}) - <span class="text-primary">${b.r} runs</span> off ${b.b} balls</button>`;
    });
    ...
    perfs.bowlers.forEach((b, i) => {
        let cleanName = b.name.replace(/[^a-zA-Z0-9]/g, '_'); let pStr = encodeURIComponent(JSON.stringify(b)).replace(/'/g, "%27");
        html += `<button type="button" class="btn-action w-100" style="background:rgba(255,255,255,0.05); text-align:left; padding:12px; border-left:3px solid #ef4444;" onclick="previewPlayerCard('${pStr}', false, '${cleanName}')"><b>${b.name}</b> (${b.team}) - <span class="text-danger">${b.w} Wkts</span> for ${b.rc} runs</button>`;
    });
    html += `</div></div>`;
...
function previewPlayerCard(playerStr, isBatter, safeName) {
...
function downloadElementAsImage(elementId, fileName, btn) {
    ...
    let origText = btn.innerText; btn.innerText = "⏳ Generating..."; btn.disabled = true; let cardEl = el(elementId); let overlay = document.createElement('div'); overlay.style.position = 'fixed'; overlay.style.top = '0'; overlay.style.left = '0'; overlay.style.width = '100vw'; overlay.style.height = '100vh'; overlay.style.background = 'rgba(0,0,0,0.85)'; overlay.style.zIndex = '99999'; overlay.style.display = 'flex'; overlay.style.alignItems = 'center'; overlay.style.justifyContent = 'center'; overlay.innerHTML = '<div style="color:white; font-size:2rem; font-weight:bold;">📸 Capturing High-Res Image...</div>'; document.body.appendChild(overlay);
    let clone = cardEl.cloneNode(true); clone.style.transform = "none"; clone.style.position = "fixed"; clone.style.left = "0"; clone.style.top = "0"; clone.style.zIndex = "99998"; let origImg = cardEl.querySelector('img'); let cloneImg = clone.querySelector('img'); if(origImg && cloneImg) { cloneImg.crossOrigin = "anonymous"; cloneImg.src = origImg.src; } document.body.appendChild(clone);
