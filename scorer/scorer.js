...
async function fetchRegistry() {
    ...
    try {
        // Requires a table named 'match_registry' with columns 'category' and 'name' (name is unique)
...
async function authenticateMatch() {
    try {
        ...
        // 🔥 CRITICAL FIX: Reset Tap-and-Fly arrays every time authentication triggers
        setupSquads = {
            A: { bench: [], xi: [], subs: [], roles: { c: null, vc: null, wk: null } },
            B: { bench: [], xi: [], subs: [], roles: { c: null, vc: null, wk: null } }
        };
...
async function loadTournamentSquads(teamId, teamKey, fallbackTeamName) {
    ...
    // 🔥 CRITICAL FIX: Ensure missing relation doesn't bypass push
...
function openCardStudio() {
    ...
    perfs.batters.forEach((b, i) => {
        ...
        html += `
        <button type="button" class="btn-action w-100" style="background:rgba(255,255,255,0.05); text-align:left; padding:12px; border-left:3px solid #38bdf8;" onclick="previewPlayerCard('${pStr}', true)">
    ...
    perfs.bowlers.forEach((b, i) => {
        let pStr = encodeURIComponent(JSON.stringify(b)).replace(/'/g, "%27");
        html += `
        <button type="button" class="btn-action w-100" style="background:rgba(255,255,255,0.05); text-align:left; padding:12px; border-left:3px solid #ef4444;" onclick="previewPlayerCard('${pStr}', false)">
